...
C9. AlphaGen ("Generating Synergistic Formulaic Alpha Collections via Reinforcement Learning," Yu, Xue, Ao et al., KDD 2023 ADS track; arXiv 2306.12964) mines formulaic alphas with RL where the reward is the performance of the downstream combination model — optimizing a synergistic SET of alphas rather than individual expressions — and beats gplearn genetic-programming, single-alpha PPO, XGBoost, LightGBM, and MLP baselines on CSI300/CSI500 Chinese A-share data. (The paper's experiments contain no deep-symbolic-regression baseline — DSR is cited only in related work; a minimal DSO baseline ships separately in the companion GitHub repo.)
Evidence: Paper/abstract: RL "prioritizes mining a synergistic set of alphas"; "the downstream combination model's performance becomes the reward signal"; experiments on CSI300 and CSI500; baselines in the experiments section are GP ("a genetic programming model using the alpha's IC as the fitness measure," implemented on the gplearn framework), PPO single-alpha RL, XGBoost, LightGBM, MLP — no DSR/DSO row in any results table (verified against the arXiv HTML experiments section); the repo README ships "modified versions of Genetic Programming (gplearn) and Deep Symbolic Regression (DSO) baselines."
...
C21. The 101 alphas define the canonical signal-form vocabulary for daily cross-sectional price-volume alphas: 4 of 101 are delay-0 (numbers 42, 48, 53, 54) and the rest delay-1; formulas combine rank, Ts_Rank, correlation, delta, sign, log, decay_linear (used in 22 of the 101 formulas, 40 in-formula occurrences) and IndNeutralize (16 formulas case-sensitively, 17 occurrences since Alpha#67 uses it twice; 18 formulas counting the lowercase "indneutralize" in Alphas 48 and 100 — the paper states expressions are case-insensitive) over open/high/low/close/volume/vwap/returns/adv{d} — e.g., Alpha#101 = ((close − open) / ((high − low) + .001)) and Alpha#3 = (−1 × correlation(rank(open), rank(volume), 10)).
Evidence: Paper footnote 11: "Four of our 101 alphas in Appendix A, namely, the alphas numbered 42, 48, 53 and 54, are delay-0 alphas"; Appendix A formulas as quoted; operator counts recomputed programmatically in revision round 1 by parsing Appendix A of the arXiv PDF into the 101 individual formulas (pypdf extraction; Alpha#101 body truncated at the A.1 operator-definitions section): decay_linear in formulas 31, 39, 57, 58, 59, 63, 66, 71, 72, 73, 76, 77, 82, 87, 88, 89, 91, 92, 93, 96, 97, 98 (22 formulas, 40 occurrences, plus 1 in the A.1 definitions = 41 total); IndNeutralize case-sensitive in formulas 58, 59, 63, 67, 69, 70, 76, 79, 80, 82, 87, 89, 90, 91, 93, 97 (16 formulas, 17 occurrences); adding lowercase uses in Alphas 48 and 100 gives 18 formulas, 21 occurrences. Counts independently match the round-1 adversarial recount.
Source: https://arxiv.org/abs/1601.00991
Confidence: high
...
C23. AlphaEvolve (Cui, Wang, Zhang et al.; SIGMOD 2021; arXiv 2103.16196) — unrelated to DeepMind's later system of the same name — used AutoML-style evolutionary search over scalar/vector/matrix operators to evolve a new class of alphas that sit between formulaic and machine-learning alphas, with the abstract claiming evolved alphas with "high returns and weak correlations" and the experiment tables reporting Sharpe ratios (best evolved alpha Sharpe 21.32 vs 4.11 for a domain-expert alpha) and low pairwise correlations.
Evidence: Abstract (verbatim, re-verified on the arXiv abs page in round 1): "a new class of alphas... which possess the strengths of these two existing classes"; "AlphaEvolve... searches from a large space of operators operating on scalar, vector, or matrix operands" (paraphrase); "Experiments show that AlphaEvolve can evolve initial alphas into the new alphas with high returns and weak correlations." The Sharpe-ratio statement is sourced to the paper's experiment tables (Tables 1–6 per the round-1 audit), not the abstract — "Sharpe ratios" does not appear in the abstract.
...
C25. gplearn-style genetic programming (symbolic regression evolving expression trees by mutation/crossover, extended with time-series operators) is the standard practitioner and academic baseline for formulaic alpha mining — attributed in the literature to Lin et al. (2019), a Huatai Securities Research Center technical report (practitioner gray literature, not peer-reviewed), as the first GP method for alpha factors — and is still being refined (e.g., warm-start GP with structural constraints on 2020–2024 Chinese A-share data, arXiv 2412.00896, Dec 2024).
Evidence: AlphaForge (arXiv 2406.18394v3), Related Work, verbatim (re-verified in round 1): "Early advancements, notably within the GPLearn package,(Lin et al. [2019a]) introduced time series operators, establishing the first genetic programming method for mining alpha factors." Corroborated by the AlphaGen paper ("[10] augmented the gplearn library with formulaic-alpha-specific time-series operators, upon which an alpha-mining framework is built," where [10] = Xiaoming Lin, Ye Chen, Ziyu Li, Kang He, 2019, Huatai Securities Research Center report) and by arXiv 2505.11122 Appendix B ("Early work like GPLearn (Lin et al. 2019) applies GP with a pre-defined set of time-series operators"). The alphagen repo ships "modified versions of Genetic Programming (gplearn) and Deep Symbolic Regression (DSO) baselines." Note: the warm-start GP paper (arXiv 2412.00896) does not contain the GPLearn attribution sentence; it is cited here only for the continued-refinement point.
Source: https://arxiv.org/html/2406.18394v3 ; https://arxiv.org/abs/2306.12964 ; https://arxiv.org/abs/2505.11122 ; https://arxiv.org/abs/2412.00896 ; https://github.com/RL-MLDM/alphagen/
Confidence: high
...
On the LLM-sentiment side, Lopez-Lira & Tang is decisive prior art for "LLM reads news, trade the score" (full-sample Sharpe 2.97 for GPT-4, pre-transaction-cost and equal-weighted — the paper itself reports 1.29 at 10 bps round-trip and 0.56 value-weighted; the separate subperiod series decays from 6.54 in 2021Q4 to 1.22 by early 2024), and their decay curve is a warning that even genuine LLM edges are arbitraged within quarters. BloombergGPT and FinGPT are domain-LLM infrastructure, not signals.
...
Adversarial audit: research/debates/llm_in_trading_round1.md (2026-07-19). 26 of 30 claims CONFIRMED; 4 objections addressed below. Claim numbering unchanged. All corrections independently re-verified by the revision agent (arXiv 1601.00991 PDF re-downloaded and Appendix A re-parsed programmatically with pypdf; arXiv abs/HTML pages re-fetched for 2103.16196, 2406.18394v3, and 2306.12964).

- C9 (OVERSTATED → fixed): removed the assertion that AlphaGen beats "deep-symbolic-regression baselines." The paper's experimental baselines are GP (gplearn, IC fitness), PPO, XGBoost, LightGBM, and MLP only; DSR is cited only in related work, and a minimal DSO baseline exists solely in the companion GitHub repo. Claim now lists the actual baselines and flags the DSO baseline as repo-only. Re-verified against the arXiv HTML experiments section.
- C21 (WRONG → fixed): corrected operator counts. decay_linear: 22 of 101 formulas (40 in-formula occurrences; 41 including the A.1 operator-definitions section), not 39. IndNeutralize: 16 formulas case-sensitively with 17 occurrences (Alpha#67 uses it twice; the old "17 formulas" was an occurrence count), 18 formulas / 21 occurrences counting lowercase "indneutralize" in Alphas 48 and 100. Counts recomputed independently from the arXiv PDF and match the adversary's recount exactly. Delay-0 list, Alpha#101, Alpha#3, and operator vocabulary were confirmed and left unchanged.
- C23 (OVERSTATED → fixed): replaced the misquoted abstract sentence. The actual abstract reads "Experiments show that AlphaEvolve can evolve initial alphas into the new alphas with high returns and weak correlations" (re-verified on the arXiv abs page); "Sharpe ratios" does not appear in the abstract. The Sharpe-ratio content is retained but now explicitly sourced to the paper's experiment tables (best evolved alpha Sharpe 21.32 vs 4.11 for a domain-expert alpha).
- C25 (OVERSTATED → fixed): re-attributed the "Early advancements ... GPLearn package (Lin et al. 2019a) ... first genetic programming method" quote to its true source, AlphaForge (arXiv 2406.18394v3, Related Work; re-verified verbatim), not the warm-start GP paper (arXiv 2412.00896), which never mentions GPLearn. Added corroborating citations (AlphaGen ref [10]; arXiv 2505.11122 Appendix B) and noted that Lin et al. 2019 is a Huatai Securities Research Center technical report (gray literature). arXiv 2412.00896 is retained only for the continued-refinement point.
- Synthesis (adversary nitpick 1, adopted): qualified the Lopez-Lira & Tang Sharpe 2.97 as pre-transaction-cost and equal-weighted (paper reports 1.29 at 10 bps round-trip, 0.56 value-weighted), and separated the full-sample Sharpe from the subperiod decay series, so REPORT.md does not inherit an unqualified number.

Status set to "draft round 1". Not marked agreed — that determination belongs to the adversary/judge.
