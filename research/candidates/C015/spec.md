...
**Convergence evidence:** the loop-1 search (different seed, 1-day hold, full train window,
full universe) independently landed its only Gate-2 survivor in the SAME family (C010 =
`roll_mean(roll_std(rng,10),63)`, val SR +0.57). Two independent searches converging on
range-volatility is family-level robustness evidence, recorded here pre-gates.
...
**Signal (exact formula).** `signal = rng.rolling(63).std()` where `rng = (high-low)/close`.
Decile long-short (long high), 5-day hold, t+2 execution, S&P 500 equities, 10 bps.
