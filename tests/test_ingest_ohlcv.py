...
class TestLoadOhlcv:
    ...
    def test_explicit_summer_and_winter_offsets_normalize_to_utc(self, tmp_path: Path) -> None:
        csv = _write(
            tmp_path,
    ...
    def test_write_normalized_failure_is_domain_error(self, tmp_path: Path) -> None:
        ...
        frame, _ = load_ohlcv(csv)
        # Missing parents are auto-created now, so fail on a directory
        # squatting on the target path instead.
        target = tmp_path / "norm.csv"
        target.mkdir()
        with pytest.raises(MappingError, match="Could not write normalized OHLCV"):
            write_normalized(frame, target)
